...
# should be C/python +1 -> (x,y) -> (x+1,y+1) or even +1.5 as this is as in ds9 convention 1-indexed
def pix2sky( fits, x, y, fitsname=None, debug_level=0 ):
    x_size=fits[0].header['NAXIS1']
    # channels=100
    y_size=fits[0].header['NAXIS2']
...
#       dim = fits[0].data.shape
    ...
    if naxes == 2 :
       ...
       (ra, dec, rubbish1 ) = wcs.all_pix2world( x, y, numpy.array([0]), 1 )
    else :
       (ra, dec, rubbish1, rubbish2 ) = wcs.all_pix2world( x, y, numpy.array([0]), numpy.array([0]), 1 )

    return (ra,dec)



def sky2pix( fits, ra, dec, fitsname=None, debug_level=0 ):
    x_size=fits[0].header['NAXIS1']
    # channels=100
    y_size=fits[0].header['NAXIS2']
