...
class CandidateCounter:
    def __init__( self, uxtime=-1, start_counter=0, sat_above_hor_all=0, sat_above_min_elev=0, random_match_probability=0, low_cand_count=-1, high_cand_count=-1, file="" ):
        self.file = file
        self.uxtime = uxtime
...
def is_plane_path( azim , alt , radius=10 ) :
   ...
   return (False,ang_dist_deg,0)
    

def read_sattest_output( filename, transient_ra_deg, transient_dec_deg, sat_radius_deg=360.00, min_elevation=0.00 ) : 
...
#      print "line = %s , words = %d" % (line,len(words))
   ...
   return ( closest_sat , min_ang_dist_arcsec , satlist, all_above_horizon_count, above_min_elevation_count )
...
def read_candidate_file( filename, candidates=None ) :
   # for time to be UTC :
   os.environ["TZ"] = "UTC"
...
# chan_204_20200530T073345_I_diff.fits 145 134 83.41 9.67 5.00 8.62 71.496394 18.913049 310.267509 25.763331
         ...
         candidates.append( skysources.Source( name=fitsname, ra = ra, dec = dec,  flux=flux, x=x, y=y, snr=snr, rms=rms, azim_deg=azim, elev_deg=elev, uxtime=uxtime, file=fitsname ) )
...
#            d=evt_time.to_datetime()
            ...
            if os.path.exists( tle_file ) :      
               ...
               if not satfile_exists or options.sat_force :
                  cmd = "sattest %.4f -tle=%s -all -mwa -qth=mro.qth -ra=%.8f  -dec=%.8f -outregfile=%.4f.reg  -outfile=%.4f.txt -print_header -no_spaces > %.4f.out 2>&1" % (mean_ux,tle_file,mean_ra_degs,mean_dec_degs,mean_ux,mean_ux,mean_ux)
                  print("DEBUG : %s" % cmd )
                  ret = os.system( cmd )
...
#               else :   
               (  closest_sat , min_ang_dist_arcsec , satellites , all_above_horizon_count, above_min_elevation_count ) = read_sattest_output( satfile, mean_ra_degs, mean_dec_degs, sat_radius_deg=options.sat_radius_deg, min_elevation=min_elevation )
         ...
         if len(satinfo_trunc) > 20 :
            satinfo_trunc = satinfo[0:20]
...
#         if classif == "TRANSIENT_CANDIDATE" : # if still not rejected by earlier criteria :
         if True : # always check PLANES :
            if (mean_az_degs >= 10 and mean_az_degs <= 140) or (mean_az_degs >= 210 and mean_az_degs <= 350) : # path1 or path2 
               (is_plane,path_ang_dist_arcsec,plane_path_index) = is_plane_path( mean_az_degs, mean_elev_degs, radius=3*options.beam_size_deg )
               if is_plane :
                  # on plane path 
                  # since two stations might be a bit out of synch min_uxtime has to be +/- integration time :   
                  ...
                  print("DEBUG : candidate on planes path detected, maximum flux within 3 beams are low_freq = %.1f [Jy] and high_freq = %.1f [Jy]" % (brightest_low.flux,brightest_high.flux))
...
#                print("WARNING : random match probability for image %s was %.8f and now is different and = %.8f - looks like bug in code, please verify !!!" % (candidates_per_image[ cand_high.file ].random_match_probability,random_match_probability))
      ...
      if brightest_low_last is not None :
         print("DEBUG_BRIGHTEST_LOW  : brightest objects in high %s is %.2f [Jy] at (%.1f,%.1f)" % (brightest_low_last.file,brightest_low_last.flux,brightest_low_last.x,brightest_low_last.y))
...
#         (n_low,last_index)  = count_candidates( candidates_low_freq , cand_low.file, start_index=0 )
      ...
      if True : # ret : - do not require the MEGA-BRIGHT ones to be in coincidence even (matching the positions very well as they are large and can be at slightly different positions in both stations !)
         ...
         if brightest_high_last is not None and brightest_low_last is not None :
            print("DEBUG_BRIGHTEST_LOW and DEBUG_BRIGHTEST_HIGH -> is RFI ( compare %.2f and %.2f vs. %.2f - both above to flag all on %s ? ) number of events low/high = %d/%d ?" % (brightest_high_last.flux,brightest_low_last.flux,options.rfi_flux_threshold,cand_high.file,candidates_per_image[ cand_high.file ].low_cand_count,candidates_per_image[ cand_high.file ].high_cand_count))
            if brightest_high_last.flux > options.rfi_flux_threshold and brightest_low_last.flux > options.rfi_flux_threshold :
               print("DEBUG : RFI detected in both low/high images ( %.4f / %.4f ) [Jy/beam] > threshold = %.4f [Jy]" % (brightest_high_last.flux,brightest_low_last.flux,options.rfi_flux_threshold))
               if candidates_per_image[ cand_high.file ].low_cand_count >= options.min_rfi_cand_count and candidates_per_image[ cand_high.file ].high_cand_count >= options.min_rfi_cand_count :
                  ...
                  # flagged = FlagRFI( coinc_candidates, cand_high.file )
                     ...
                     print("DEBUG : RFI detected and number of events on low/high = %d / %d > max = %d -> high file %s added to flagged list !" % (candidates_per_image[ cand_high.file ].low_cand_count,candidates_per_image[ cand_high.file ].high_cand_count,options.min_rfi_cand_count,cand_high.file))
...
def parse_options(idx=0):
   ...
   parser.add_option('--rfi_flux_threshold','--rfi_thresh','--rfi',dest="rfi_flux_threshold",default=2000,help="RFI threshold [default %default]",type="float")
   parser.add_option('--min_rfi_cand_count','--max_image_cand_count','--rfi_cand_count',dest="min_rfi_cand_count",default=20,help="RFI threshold [default %default]",type="int")
...
if __name__ == "__main__":
   ...
   # def read_sattest_output( filename, transient_ra_deg, transient_dec_deg, sat_radius_deg=360.00, min_elevation=0.00 ) :
   read_sattest_output("1586615665.0000.txt",0.00,0.00)