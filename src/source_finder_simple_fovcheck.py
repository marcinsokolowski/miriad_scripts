...
#         for x in range(border,x_size-border) :         
      pixels = []
      while len(pixels) < options.monte_carlo_pixels :
          # 116 
          x = random.randrange(0,x_size)
          y = random.randrange(0,y_size)
          
          dist_center = math.sqrt( (x-x_center)*(x-x_center) + (y-y_center)*(y-y_center) )
          if dist_center <= options.fiducial_radius :
      ...
      for pixel in pixels :
            ...
            if options.debug_level > 0 or (all_pixels % 100) == 0 :
               print("DEBUG : processing pixel (%d,%d) - %d / %d" % (x,y,all_pixels,len(pixels)))
            all_pixels += 1 
...
#                   progress = ( float(y-options.window[1]) / float(all_y_to_process) )*100.00
            ...
            if value > thresh_value or True : # always (True)                          
#               n_above_threshold += 1 
...
#               min_elev = 25
#               count_above=0
#               for yy in range(0,y_size) :
#                  for xx in range(0,x_size) :
#                     if alt[yy,xx] > min_elev :
#                        count_above += 1
                        
               print("Number of pixels above elevation %.2f [deg] = %d out of total %d pixels" % (min_elev,count_above,(x_size*y_size)))
               ...
               new_transient=True
               if options.filtering_enabled or True :
                  if alt <= min_alt :
                     if options.debug_level > 0 :
                  ...
                  if gleam_sources is not None and len(gleam_sources) > 0 :
                     ...
                     if gleam_source is not None :
                        print("\tFound in gleam source %s with flux = %.2f in distance = %.2f [arcsec] -> skiped" % (gleam_source,gleam_flux,gleam_dist_arcsec))
                        new_transient=False
               else :
                  if options.debug_level > 0 :
                     print("WARNING : filtering / criteria are disabled")

#                all_analysed_pixels += 1
               ...
               if True : 
                  ...
                  if check_sun :
                     ...
                     if sun_dist is not None :
                        ...
                        if sun_dist_deg <= options.sun_radius :
                           print("\tSource at (%d,%d) (ra,dec) = (%.4f,%.4f) [deg] identified as Sun -> skiped" % (x,y,ra_deg,dec_deg))
                           is_sun = True
                           closest_source = sun
                  ...
                  if not is_sun and options.check_ateam_sources and options.filtering_enabled :
                     ...
                     if found_src :
                        print("\tFound a nearby source %s -> rejecting candidate -> skiped" % (closest_source.name))
                        
                  if is_sun or found_src or rejected :                     
                     continue
...
#            print("DEBUG : finished y = %d, number of pixels above threshold = %d -> number of sources = %d" % (y,n_above_threshold,len(candidates)))
      ...
      print("File %s ( %.4f ) : all pixels = %d , %d pixels above min_elev = %.2f [deg] , analysed pixels %d -> acceptance = %.4f" % (fitsname,uxtime,all_pixels,all_pixels_above_min_alt,min_alt,all_analysed_pixels,(float(all_analysed_pixels)/float(all_pixels_above_min_alt))))
